...
local function ToggleBlipsAndNames(isBlips)
    if isBlips then
        ...
        RSGCore.Functions.Notify(locale(message), ShowBlips and "success" or "error")
    else
        ShowNames = not ShowNames
        NetCheck2 = ShowNames
        local message = ShowNames and "names_activated" or "names_deactivated"
        RSGCore.Functions.Notify(locale(message), ShowNames and "success" or "error")
    end
...
local function preparePlayers()
    currentPlayers = {}
    Wait(100)
    currentPlayers = lib.callback.await('yh-admin:callback:GetPlayers')
end


RegisterNetEvent('yh-admin:client:toggleBlips', function(data)
...
RegisterNetEvent('yh-admin:client:toggleNames', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent("yh-admin:client:MutePlayer", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent('yh-admin:client:removeStress', function(data)
