...
local function teleport(x, y, z)
    if cache.vehicle then
        return SetEntityCoords(cache.ped, x, y, z)
    end
...
-- Teleport to player
RegisterNetEvent('yh-admin:client:TeleportToPlayer', function(coords)
    lastCoords = GetEntityCoords(cache.ped)
    SetEntityCoords(cache.ped, coords.x, coords.y, coords.z)
end)

-- Teleport to coords
RegisterNetEvent('yh-admin:client:TeleportToCoords', function(data, selectedData)
...
-- Teleport to Locaton
RegisterNetEvent('yh-admin:client:TeleportToLocation', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Teleport back
RegisterNetEvent('yh-admin:client:TeleportBack', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
