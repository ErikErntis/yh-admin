...
function CheckPerms(perms)
	return lib.callback.await('yh-admin:callback:CheckPerms', false, perms)
end
...
function Log(title, message)
	TriggerServerEvent("rsg-log:server:CreateLog", "yh-admin", title, "red", message)
	TriggerEvent('rsg-log:server:CreateLog', 'yh-admin', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), 'rsg-adminmenu', "system banned you for inappropriate use"), true)
end
