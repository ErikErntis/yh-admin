...
local function noPerms(source)
    return RSGCore.Functions.Notify(source, "You are not Admin or God.", 'error')
end
...
function CheckAlreadyPlate(plate)
    local vPlate = RSGCore.Shared.Trim(plate)
...
lib.callback.register('yh-admin:callback:CheckPerms', function(source, perms)
    return CheckPerms(perms)
end)

lib.callback.register('yh-admin:callback:CheckAlreadyPlate', function(_, vPlate)
...
function CheckRoutingbucket(source, target)
    ...
    RSGCore.Functions.Notify(source, locale("bucket_set", targetBucket), 'error', 7500)
end
